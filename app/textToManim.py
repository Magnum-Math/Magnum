...
def generateTextMobject(line):
    retval = ' = TextMobject("' + line.split("\n")[0]+ '" )\n\t\t'
    return retval
...
def latex2Manim(latexArr, query = "" ,func = None):
    ...
    if len(latexArr) >= 1 :
        retval = retval + 'Solve' + generateTextMobject(latexArr[0])
        retval = retval + 'Solve.to_edge(UP)\n\t\t'+'self.play(Write(Solve))\n\t\t'
        retval = retval + "align_mark = TextMobject( r'abs', fill_opacity=0.00,height=0.5)\n\t\t"
        ...
        for i in range(1, len(latexArr)):
            retval = retval + 'R' + str(i-1) + generateTextMobject(latexArr[i])
        ...
        if graph != True:
            ...
            #retval += "R0" + generateTextMobject(latexArr[0])
